...
$(function () {
    ...
    ajaxService.newsList(reqdata, (res) => {
      if (res.code == 0) {
        ...
        for (let i = 0; i < res.data.length; i++) {
          ...
          html += `<a href="/en/about_newsdetail.html?id=${res.data[i].id}" class="news-item ${i%2==1?'bl-mr0':''}">
          <img src="${res.data[i].image}" class="news-item-img" alt="">
  ...
  function getCategoryList() {
    ajaxService.categoryList(function (res) {
      if (res.code == 0) {
        let list = `<li><a href="javascript:;" >all</a></li>`;
        for (let i = 0; i < res.data.length; i++) {
    ...
    $('.news-cen-list').on("click", 'a', function () {
      let url =`/en/about_news.html?cuPage=1`;
  ...
  function getStickyList() {
    ajaxService.stickyList({
      ...
      if (res.code == 0) {
        let list = ``;
        for (let i = 0; i < res.data.length; i++) {
          list += `<a href="/en/about_newsdetail.html?id=${res.data[i].id}">${res.data[i].sticky_title}</a>`;
        }
